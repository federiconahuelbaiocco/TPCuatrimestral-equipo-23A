var ConsultoriosModule = (function() {
    'use strict';
    ...
    function actualizarEstadoSidebar(checkbox) {
        ...
        if (checkbox.checked) {
            ...
            statusDescription.textContent = 'Un consultorio inactivo no recibira turnos ni asignaciones';
            statusMessage.className = 'sidebar-status-message text-danger';
        }
    }

    return {
        cargarConsultorio: cargarConsultorio,
...
function cargarConsultorio(id, nombre, activo) {
    ConsultoriosModule.cargarConsultorio(id, nombre, activo);
}

function limpiarFormulario() {
    if (document.querySelector('[id$="hfConsultorioId"]')) {
        ConsultoriosModule.limpiarFormulario();
    }
}

function actualizarEstadoSidebar(checkbox) {
    if (document.querySelector('[id$="hfConsultorioId"]')) {
        ConsultoriosModule.actualizarEstadoSidebar(checkbox);
    }
}
